...
1. Use `erc8004_status` for persisted state, `erc8004_refresh` when the operator asks for a
   current balance, says funds were sent, or asks the Tentacle to resume, and `erc8004_republish`
   (or `/registry-republish`) when the operator asks to update, repair, or republish this Tentacle's
   on-chain name, avatar, or metadata. Refresh and republish perform bounded live Base reconciliation.
   Slash commands remain deterministic recovery controls, not the ordinary natural-language path.
